 ...
 * 팬풀 수정 요청 파라미터
 */
interface UpdateFanpoolRequest {
	id: string; // 수정할 팬풀의 ID
	title?: string;
 ...
 * 팬풀 수정 요청
 *
 * @param {UpdateFanpoolRequest} requestParameters 팬풀 수정에 필요한 요청 파라미터
 * @returns {Promise<void>} 성공 시 빈 응답
 */
const patchFanpool = async (
	requestParameters: UpdateFanpoolRequest
): Promise<void> => {
	// PATCH 요청으로 팬풀 정보를 body에 전달
	await apiClient.patch(`/fanpool/${requestParameters.id}`, {
