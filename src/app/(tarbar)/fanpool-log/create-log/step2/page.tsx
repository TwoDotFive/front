					...
					if (stadium) {
						setSelectedItems((prevItems) => {
							const isAlreadySelected = prevItems.some(
								(item) => item.contentId === stadium.contentId
							);
							return isAlreadySelected ? prevItems : [...prevItems, stadium];
						});
					}
				});
			} else {
				setSelectedItems((prevItems) => {
					const newSelectedItems = schedules.map((schedule) => schedule.place);
					const updatedItems = prevItems.filter(
						(item) =>
							!newSelectedItems.some(
								(newItem) => newItem.contentId === item.contentId
							)
					);
					return [...updatedItems, ...newSelectedItems];
				});
			}
